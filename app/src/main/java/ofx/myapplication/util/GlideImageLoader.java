package ofx.myapplication.util;
...
public class GlideImageLoader extends ImageLoader {
    ...
    public void displayImage(Context context, Object path, ImageView imageView) {
        Glide.with(context).load(path).centerCrop().error(R.drawable.timg).into(imageView);
    }

    //使用Glide给RelativeLayout设置背景图
    public static void showImageView(Context context, String url, final RelativeLayout bgLayout) {
        Glide.with(context).load(url).asBitmap().error(R.drawable.timg)// 设置错误图片

                .diskCacheStrategy(DiskCacheStrategy.RESULT)// 缓存修改过的图片
                .transform(new BlurTransformation(context))// 高斯模糊处理
                .placeholder(R.drawable.timg)// 设置占位图
    ...
    static class BlurTransformation implements Transformation<Bitmap> {
